...
class MissingColumnException(Exception):
    def __init__(self, column_name):
        self.message = f"Input file is missing '{column_name}' column"
...
def validate_input_file(input_file: IOBase, algorithms: list[str]) -> None:
    ...
    columns_string = input_file.readlines(1)[0].decode('utf-8').replace('\n', '')

    for obligatory_column in list_of_obligatory_columns:
        if obligatory_column not in columns_string.split('\t'):
            raise MissingColumnException(obligatory_column)
    ...
    for row in input_file.readlines():
        if row.decode('utf-8').count('\t') != number_of_tabulators_in_row:
